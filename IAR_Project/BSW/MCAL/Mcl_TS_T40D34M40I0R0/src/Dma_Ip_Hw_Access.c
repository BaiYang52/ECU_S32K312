...
#define DMA_IP_HW_ACCESS_SW_MAJOR_VERSION_C                4
...
#if (DMA_IP_BANDWIDTH_CONTROL_IS_AVAILABLE == STD_OFF) 
void HwAccDmaCh_BandwithControlrDummyFunction(const uint32 LocHwVers, Dma_Ip_TcdRegType * pxLocTcd, const uint32 LocValue)
{
    /* Set transfer dummy function */
    (void)LocHwVers;
    (void)pxLocTcd;
...
{
#if ((STD_ON == DMA_IP_HWV2_IS_AVAILABLE) || (STD_ON == DMA_IP_HWV5_IS_AVAILABLE))
    uint32 hwLinkCh;
    ...
    if(DMA_IP_HARDWARE_VERSION_3 == HwVersId)
    {
        ...
        {
            hwv3AccInlineDmaCh_SetTCDParams(pxLocScatterGather->Stcd, pxLocScatterGather, ElementId);
        }
    }
    else
    {
